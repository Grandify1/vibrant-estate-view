...
export default function PropertyDetail({ property, isOpen, onClose }: PropertyDetailProps) {
    ...
    return (
      <div className={`relative ${className}`} onClick={onClick}>
        ...
        <img
          src={src}
          alt={alt}
          className={`w-full h-full object-cover ${loaded ? 'opacity-100' : 'opacity-0'} ${onClick ? 'cursor-pointer' : ''}`}
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="sm:max-w-[900px] min-h-[600px] max-h-[90vh] overflow-y-auto p-0 border bg-white">
          ...
          <div className="p-4 md:p-6 space-y-8">
            ...
            {agent && (
              <div className="bg-gray-50 rounded-lg p-4 md:p-6">
                <h3 className="text-xl font-semibold mb-4">Ihr Ansprechpartner</h3>
                <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                  ...
                  <div>
                    <h4 className="text-lg font-medium">{agent.first_name} {agent.last_name}</h4>
                    {agent.position && <p className="text-gray-600">{agent.position}</p>}
      ...
      {/* Lightbox für Bilder */}
      <Lightbox 
        images={images.map(img => ({ id: img.id, url: getValidImageUrl(img.url) }))} 
